...
public class FolderUtils {
    ...
    public boolean downLoadDirectory(String localDirectoryPath, String remoteDirectory) {
        try {
            ...
            this.ftpClient.mlistDir(remoteDirectory);
            for (int currentFile = 0; currentFile < allFile.length; currentFile++) {
                if (allFile[currentFile].isDirectory()) {
                    String strremoteDirectoryPath = remoteDirectory + "/" + allFile[currentFile].getName();
                    System.out.println(strremoteDirectoryPath + "----" + localDirectoryPath);
                    String fileName1 = new File(strremoteDirectoryPath).getName();
                    String local1 = localDirectoryPath + fileName1 + "\\";
                    new File(local1).mkdirs();
                }
            }
            for (int currentFile = 0; currentFile < allFile.length; currentFile++) {
                if (!allFile[currentFile].isDirectory()) {
